...
class EcsTaskRunInternalOperator(operatorName: String, context: OperatorContext, systemConfig: Config, templateEngine: TemplateEngine)
  ...
  val placementConstraints: Seq[PlacementConstraint] =
    params.parseListOrGetEmpty("placement_constraints", classOf[Config]).asScala.map(configurePlacementConstraint).map(_.get)

  val placementStrategy: Seq[PlacementStrategy] =
    params.parseListOrGetEmpty("placement_strategy", classOf[Config]).asScala.map(configurePlacementStrategy).map(_.get)
  ...
  val tags: Seq[Tag] = params.getMapOrEmpty("tags", classOf[String], classOf[String]).asScala.map(t => new Tag().withKey(t._1).withValue(t._2)).toSeq
  ...
  protected def configureAwsVpcConfiguration(c: Config): Optional[AwsVpcConfiguration] = {
    ...
    val securityGroups: Seq[String] = c.parseListOrGetEmpty("security_groups", classOf[String]).asScala
    val subnets: Seq[String] = c.parseListOrGetEmpty("subnets", classOf[String]).asScala
  ...
  protected def configureTaskOverride(c: Config): Optional[TaskOverride] = {
    ...
    val containerOverrides: Seq[ContainerOverride] =
      c.parseListOrGetEmpty("container_overrides", classOf[Config]).asScala.map(configureContainerOverride).map(_.get)
  ...
  protected def requestTaskRun(req: RunTaskRequest): RunTaskResult = {
    ...
    def runUntilSuccess(numRetry: Int = 0): RunTaskResult = {
      ...
      val failures: Seq[Failure] = lastResult.getFailures.asScala
