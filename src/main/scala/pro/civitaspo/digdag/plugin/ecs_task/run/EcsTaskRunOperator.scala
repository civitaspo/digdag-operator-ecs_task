...
import io.digdag.spi.{ImmutableTaskResult, OperatorContext, PrivilegedVariables, SecretProvider, TaskRequest, TaskResult, TemplateEngine}
...
class EcsTaskRunOperator(operatorName: String, context: OperatorContext, systemConfig: Config, templateEngine: TemplateEngine)
  ...
  // For the other operators configuration
  val taskDefForRegister: Optional[Config] = params.getOptionalNested("def")
  ...
  // For ecs_task.run operator configuration
  ...
  val taskDefinition: Optional[String] = params.getOptional("task_definition", classOf[String]) // generated by ecs_task.register> operator if not set.

  protected def buildRunTaskRequest(taskDefArn: String): RunTaskRequest = {
    ...
    req.setTaskDefinition(taskDefArn)
  ...
  override def runTask(): TaskResult = {
    if (!taskDefForRegister.isPresent && !taskDefinition.isPresent) throw new ConfigException("'def' or 'task_definition' must be set.")
    if (taskDefForRegister.isPresent && taskDefinition.isPresent) throw new ConfigException("'def' and 'task_definition' cannot be used at the same time.")

    val taskDefArn: String =
      if (taskDefinition.isPresent) taskDefinition.get
      else {
        val taskResult: TaskResult = runEcsTaskRegister(taskDefForRegister.get)
        taskResult.getStoreParams.getNested("last_ecs_task_register").get("task_definition_arn", classOf[String])
      }

    val req: RunTaskRequest = buildRunTaskRequest(taskDefArn)
  ...
  protected def runEcsTaskRegister(registerConfig: Config): TaskResult = {
    val config = params.deepCopy()
    config.getKeys.asScala.filter(_.startsWith("_")).foreach(config.remove)
    config.setNested("_command", registerConfig)
    config.set("_type", "ecs_task.register")

    val newContext: OperatorContext = new OperatorContext {
      override def getProjectPath: Path = context.getProjectPath
      override def getTaskRequest: TaskRequest =
        TaskRequest
          .builder()
          .from(context.getTaskRequest)
          .config(config)
          .build()
      override def getSecrets: SecretProvider = context.getSecrets
      override def getPrivilegedVariables: PrivilegedVariables = context.getPrivilegedVariables
    }

    val operator: EcsTaskRegisterOperator =
      new EcsTaskRegisterOperator(operatorName = "ecs_task.register", context = newContext, systemConfig = systemConfig, templateEngine = templateEngine)
