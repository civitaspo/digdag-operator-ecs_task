...
class EcsTaskRegisterOperator(operatorName: String, context: OperatorContext, systemConfig: Config, templateEngine: TemplateEngine)
  ...
  protected def configureContainerDefinition(c: Config): Optional[ContainerDefinition] = {
    ...
    val dependsOn: Seq[ContainerDependency] = c.parseListOrGetEmpty("depends_on", classOf[Config]).asScala.map(configureDependsOn).map(_.get).toSeq
  ...
  protected def configureDependsOn(c: Config): Optional[ContainerDependency] = {
    ...
    val cd: ContainerDependency = new ContainerDependency()
    if (containerName.isPresent) cd.setContainerName(containerName.get)
